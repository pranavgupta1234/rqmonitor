...
$(document).ready(function () {
    ...
    function action_modal_onshow(modal_selector) {
        $(modal_selector).on('show.bs.modal', function (event) {
            ...
            if (target_class === 'queue') {
                if (action === 'empty') {
                    update_modal_info(modal, 'Confirm to empty ' + target_id, target_class, target_id, action, 'All valid jobs currently on queue will be removed!')
    ...
    function action_modal_onconfirm(modal_selector) {
        $(modal_selector).on('click', '.confirm', function (event) {
            ...
            if (target_class == 'queue') {
                if (task == 'empty') {
                    ajax_action("POST", "{{ url_for('rqmonitor.empty_queue_api') }}", { 'queue_id': target_id, 'csrfmiddlewaretoken': '{{ csrf_token }}' }, queues_table, modal);
                } else if (task == 'delete') {
                    ajax_action("POST", "{{ url_for('rqmonitor.delete_queue_api') }}", { 'queue_id': target_id, 'csrfmiddlewaretoken': '{{ csrf_token }}' }, queues_table, modal);
                } else if (task == 'deleteall') {
                    ajax_action("POST", "{{ url_for('rqmonitor.delete_all_queues_api') }}", { 'csrfmiddlewaretoken': '{{ csrf_token }}' }, queues_table, modal);
                } else if (task == 'emptyall') {
                    ajax_action("POST", "{{ url_for('rqmonitor.empty_all_queues_api') }}", { 'csrfmiddlewaretoken': '{{ csrf_token }}' }, queues_table, modal);
                }
            } else if (target_class == 'job') {
                if (task === 'requeue') {
                    ajax_action("POST", "{{ url_for('rqmonitor.requeue_job_api') }}", { 'job_id': target_id, 'csrfmiddlewaretoken': '{{ csrf_token }}' }, jobs_table, modal);
                } else if (task === "delete") {
                    ajax_action("POST", "{{ url_for('rqmonitor.delete_job_api') }}", { 'job_id': target_id, 'csrfmiddlewaretoken': '{{ csrf_token }}' }, jobs_table, modal);
                } else if (task === "cancel") {
                    ajax_action("POST", "{{ url_for('rqmonitor.cancel_job_api') }}", { 'job_id': target_id, 'csrfmiddlewaretoken': '{{ csrf_token }}' }, jobs_table, modal);
                } else if (task === "deleteall") {
                    ajax_action("POST", "{{ url_for('rqmonitor.delete_all_jobs_api') }}", {
                        'queues': get_checked_queues(),
                        'jobstatus': get_checked_job_status(),
                        'csrfmiddlewaretoken': '{{ csrf_token }}'
                    }, jobs_table, modal);
                } else if (task === "requeueall") {
                    ajax_action("POST", "{{ url_for('rqmonitor.requeue_failed_jobs_api') }}", {
                        'queues': get_checked_queues(),
                        'jobstatus': get_checked_job_status(),
                        'csrfmiddlewaretoken': '{{ csrf_token }}'
                    }, jobs_table, modal);
                } else if (task === "cancelall") {
                    ajax_action("POST", "{{ url_for('rqmonitor.cancel_queued_jobs_api') }}", {
                        'queues': get_checked_queues(),
                        'jobstatus': get_checked_job_status(),
                        'csrfmiddlewaretoken': '{{ csrf_token }}'
                    }, jobs_table, modal);
    ...
    var refresh_content = setInterval(function () {
    ...
    $('#redis_memory_refresh').on('click', function () {
        ...
        $.get({
            url: "{{ url_for('rqmonitor.redis_memory_api') }}",
            cache: false
        }).then(function (data) {
    ...
    action_modal_onshow('#confirmation');
    action_modal_onconfirm('#confirmation');
    ...
    function setup_jobs_datatable() {
        ...
        */
        ...
        $.get({
            url: "{{ url_for('static', filename='nunjucks/job_info.html') }}",
            cache: false
        }).then(function (data) {
