...
class TestBlueprintViews(RQMonitorTestCase):
    ...
    def test_queue_empty_api_via_get(self):
        ...
        response = self.client.get(
            "/queues/empty", query_string={"queue_id": some_queue.name}
        )
    ...
    def test_queue_delete_api(self):
        ...
        response = self.client.post(
            "/queues/delete", data={"queue_id": some_queue.name}
        )
        self.assertEqual(response.status_code, HTTP_OK)

        queues_list_response = self.client.get(
            "/queues", query_string={"queue_id": some_queue.name}
        )
    ...
    def test_queues_delete_multiple(self):
        ...
        for queue in some_queues_instances:
            job = Job.create(
                func=fixtures.some_calculation, args=(3, 4), kwargs=dict(z=2)
            )
